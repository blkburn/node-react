...
class get_sheets:
    ...
    def pull(self, sheet_name, is_df = True):
        try:
            resp = requests.get('http://localhost:6000/v1/{}?sheet={}'.format(self.id, sheet_name), headers={'Authorization': 'Bearer {}'.format(self.token)})
            resp = resp.json()
            if (resp['success']):
                data = resp['data']
...
def run(ch, method, props, body):
    ...
    if False: #command == 'none':
        ...
        gsheets = get_sheets('6d928e8fvRZ1JGSA', cm)

        lockedSheet = gsheets.pull('LOCKED', False)
        # if lockedSheet == None:
        #     return

        isLocked = lockedSheet['Locked'][0]
        ...
        if (command == 'VERIFY_SHEET'):
            ...
            if (command == 'GET_SCHED_REQS' or command == 'GET_SCHEDULE' or command == 'GET_REQUESTS'):
                ...
                objective = objective[0:raw.shape[0]]
                ...
                staff_hours['Color'][staff_hours['Color'].isna()]='#000000'
                shifts['Color'][shifts['Color'].isna()]='#000000'
                shifts_nc['Color'][shifts_nc['Color'].isna()]='#000000'
